...
pub fn update_account_quota(account_id: &str, quota: QuotaData) -> Result<(), String> {
    ...
    let mut auto_disabled_by_consumption = false;

    // --- Auto-disable on quota consumption threshold start ---
    if let Ok(config) = crate::modules::config::load_app_config() {
        if config.proxy.auto_disable_on_consumption && !account.disabled && !account.proxy_disabled
        {
            if let Some(ref q) = account.quota {
                let consumption_percent = config
                    .proxy
                    .auto_disable_consumption_percent
                    .clamp(1.0, 99.0);
                let threshold_remaining = 100.0 - consumption_percent;
                let has_threshold_consumed = q
                    .models
                    .iter()
                    .any(|m| (m.percentage as f64) <= threshold_remaining);

                if has_threshold_consumed {
                    account.proxy_disabled = true;
                    account.proxy_disabled_reason =
                        Some(format!("quota_consumption_{:.2}%", consumption_percent));
                    account.proxy_disabled_at = Some(Utc::now().timestamp());
                    auto_disabled_by_consumption = true;

                    crate::modules::logger::log_warn(&format!(
                        "[Quota] Auto-disabled proxy for {}: consumption reached {:.2}%",
                        account.email, consumption_percent
                    ));
                }
            }
        }
    }
    // --- Auto-disable on quota consumption threshold end ---
    ...
    if auto_disabled_by_consumption {
        crate::modules::logger::log_info(&format!(
            "[Quota] Account {} disabled by consumption threshold",
            account.email
        ));
    }
